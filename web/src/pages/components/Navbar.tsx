    ...
    <Layout.Header style={{ background: currentTheme === 'dark' ? '#1f1f1f' : '#0088CC', padding: '0 20px' }}>
      <div key="logo" className="logo" style={{ marginRight: '30px' }}>
        <Link to="/dashboard" style={{ color: '#fff' }}>
          <img src="https://www.onlypacks.club/wp-content/uploads/2020/04/cropped-logo-onlypacks-2.png" style={{ height: '24px' }} />
        </Link>
      </div>
      ...
      <Menu selectable={false} overflowedIndicator={<MenuOutlined />} mode="horizontal" triggerSubMenuAction="click" theme={currentTheme === 'dark' ? 'light' : 'dark'}
        ...
        <Menu.Item onClick={() => window.open('https://onlypacks.club', '_blank')} key="blog">BLOG</Menu.Item>
        <Menu.Item onClick={() => window.open('https://foro.onlypacks.club', '_blank')} key="foro">FORO</Menu.Item>
        <Menu.Item onClick={() => window.open('https://short.onlypacks.club', '_blank')} key="short">ACORTADOR</Menu.Item>
        <Menu.Item onClick={() => window.open('https://t.me/onlypacksfree', '_blank')} key="channel">CANAL</Menu.Item>
        <Menu.Item onClick={() => window.open('https://t.me/OnlypacksChat', '_blank')} key="group">GRUPO</Menu.Item>
      </Menu>
    </Layout.Header>
